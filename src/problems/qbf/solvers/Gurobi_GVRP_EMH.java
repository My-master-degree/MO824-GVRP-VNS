...
//		afss
		for (Integer afs : problem.rechargeStationsRefuelingTime.keySet()) {
			this.nodesShortestPathFuelConsumption.put(afs, problem.distanceMatrix[afs][0] * problem.vehicleConsumptionRate);
		}
		for (Integer customer : problem.customersDemands.keySet()) {
			this.nodesShortestPathFuelConsumption.put(customer, problem.distanceMatrix[customer][0] * problem.vehicleConsumptionRate);
		}
	}
	
	public Gurobi_GVRP_EMH(String filename, GVRPInstanceReader gvrpInstanceReader) throws IOException {
		this.problem = new GVRP(filename, gvrpInstanceReader);		
		this.init();
	}	
	
	public Gurobi_GVRP_EMH(GVRP problem) throws IOException {
...
//		params
		int numberOfDummies = (problem.customersSize - 1) * 2 ;
//		extend graph
...
//		constraint (9): t_{p_{0j}} \leq \tau_{j} \leq T_{max} - t_{p_{j0}} - p_j \forall j \in V' \ {0}
		for (Integer j : v_line) {
			if (!j.equals(0)) {
				int j_line = j >= problem.size ? this.getAFSByVisit(j) : j;
				...
				Double serviceTime = problem.rechargeStationsRefuelingTime.get(j_line) != null ? 
						problem.rechargeStationsRefuelingTime.get(j_line) : problem.customersServiceTime.get(j_line);
				Double time = this.nodesShortestPathTimeConsumption.get(j_line) - serviceTime;
